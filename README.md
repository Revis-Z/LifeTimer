# LifeTimer - 励志闹钟应用 (v1.5)

## 项目概述
LifeTimer是一款专注于励志语音唤醒的iOS闹钟应用。通过高质量的励志语音内容替代传统闹铃声，为用户提供积极正面的起床体验。应用采用极简设计理念，专注于核心功能，为有自我提升需求的用户群体提供情感价值和动力激发。

## 技术选型
- **开发框架**: SwiftUI (主要)
- **数据持久化**: Core Data
- **音频/视频**: AVFoundation (AVPlayer, AVAudioPlayer)
...
## 数据模型
- **Alarm**: 闹钟实体 (时间、重复模式、启用状态)
- **VoiceContent**: 语音内容实体 (标题、文件路径)
- **VideoContent**: 视频背景实体
...
### 🧹 功能精简与代码清理
- **🗑️ 移除应用设置页面**: 彻底删除了应用设置页面 (`SettingsView`)，精简应用架构
  - **原因**: 简化应用功能，专注于核心的闹钟体验
  - **影响**: 移除了通知权限手动设置、触觉反馈开关等配置项（使用默认值）

- **🗑️ 移除语音内容管理**: 删除了语音内容管理页面 (`VoiceContentView`)
  - **原因**: 作为设置页面的子功能一并移除
  - **影响**: 用户无法手动浏览和播放语音库，仅在闹钟响起时随机播放

- **🔧 主页面重构**: 清理了 `MainView` 中与设置相关的入口和逻辑
  - **导航栏**: 移除了底部的 "Profile" 按钮
  - **状态管理**: 删除了 `showingSettings` 等相关状态变量
...
## 版本历史
### v1.5 - 精简优化版本 (2026/01/13)
- ✅ **架构精简**: 移除了应用设置模块和语音内容管理模块
- ✅ **代码清理**: 删除了冗余的视图文件和相关逻辑
- ✅ **界面优化**: 简化了底部导航栏，专注于核心功能
...
**当前版本**: v1.5 - 精简优化版本
**最后更新**: 2026年01月13日
**主要特色**: 随机视频/音频播放、固定励志句子显示、自动启用闹钟、极简交互设计
